...
def next_perfect_square(score):
    if perfect_square(score):
        score = int((sqrt(score)+1)*(sqrt(score)+1))
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < 100:
        ...
        while opponent_score<100:
            if strategy(score,opponent_score) != value:
                return False
            opponent_score+=1
        score+=1
    return True 
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    rolls = 1 
    best_score = 0 
    score_current = make_averaged(roll_dice, total_samples)
    while counter<=10:
        score = score_current(counter,dice)
        if score > best_score:
            rolls = counter
            best_score = score 
        counter+=1
